...
const { eliminarEstatusCotizacion } = estatusCotizacionStore();
const { setEstatusCotizacion } = estatusCotizacionStore();
const { obtenerEstatusCotizacion, agregarEstatusCotizacion, actualizarEstatusCotizacion } = estatusCotizacionStore();
...
const consultarEstCot = async () => {
  try {
    const estCotizacion = await obtenerEstatusCotizacion()
    console.log(estCotizacion.data.body)
    estCotArray.value = estCotizacion.data.body //guardo tofo
    estCotDesplegados.value = estCotizacion.data.body //filtrado
...
const eliminarEstatusCot = async (IdEstatusCotizacion) => {
  try {
    await eliminarEstatusCotizacion(IdEstatusCotizacion);
...
async function modificarEstCot(idEstatusCotizacion, Descripcion, idEstatusActividad) {
  try {
    await actualizarEstatusCotizacion(idEstatusCotizacion, Descripcion, idEstatusActividad);
    estCotNuevo.value = ""
    botonActualizar.value = false;
    idBotonActualizar.value = -1;
    this.consultarEstCot();
...
function actualizarTabla(nombre) {
  ...
  if (nombre.trim() == "") { //recorta los espacios
    ...
    estCotArray.value.forEach((element) => { //recorre el elemento
...
const revisarEstCot = async () => {
  if (estCotNuevo.value.trim() == "") {
    deshabilitado.value = true;
    return;
  }
  try {
    for (var j in estCotArray.value) {
      if (
        estCotArray.value[j].Descripcion.toLowerCase() == estCotNuevo.value.trim().toLowerCase()
      ) {
        repetido.value = true;
        deshabilitado.value = true;
        return true;
  ...
  <div class="container-fluid">
    ...
    <div class="row">
      ...
      <div class="col-8 mb-3 pt-5">
        <div class="row align-items-end">
          <p class="italika" style="font-size: 50px">Estatus de cotización</p>
    ...
    <div class="row mb-5">
      ...
      <div class="col-6 ms-4">
        <input type="text" ref="inputRol" class="form-control" @input="revisarEstCot()" v-model="estCotNuevo" />
    ...
    <table class="table table-hover table-striped text-center mt-4 mx-auto" style="width: 900px">
      <thead>
        <tr style="background-color: #2b4677; color: white">
          <th scope="col">Estatus de cotización</th>
          <th scope="col" style="width: 200px"></th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="estatusCotizacion in estCotDesplegados ">

          <td>
            {{ estatusCotizacion.Descripcion }}
          </td>
          <th scope="row">
            <div class="align-items-center">
              ...
              <button :id="estatusCotizacion.idEstatusCotizacion"
                :class="[botonActualizar && idBotonActualizar == estatusCotizacion.idEstatusCotizacion ? 'btn btn-primary mx-1' : 'btn btn-warning mx-1']"
                type="submit" style="border-color: #ffbe16; height: 37px"
                @click="modificarNombreEstCot(estatusCotizacion.Descripcion, estatusCotizacion.idEstatusCotizacion)">
                <i
                  :class="[botonActualizar && idBotonActualizar == estatusCotizacion.idEstatusCotizacion ? 'fa-solid fa-clock-rotate-left' : 'fa-solid fa-pen-to-square']"></i>
