...
const { obtenerEstatusCotizacion } = estatusCotizacionStore();
...
//const cotizacionesFiltrados = ref([]);
...
var modal;
//variables reactivas cotizacion
const cotizacionesFiltrados = ref([]);
const cotizacionesDesplegados = ref([]);
const cotizaciones= ref ([]);
const clientes2 = ref([]);
const usuarios2 = ref([]);
const asesores = ref([]);
const estatusCotizacion = ref([]);
const creditos = ref([]);
const nombre2 = ref("");
const idCotizacionEl = ref();
const tablaLista2 = ref(false);
...
onMounted(async () => {
  clientes.value = await obtenerTodosClientes();
  clientes.value = clientes.value.data.body;
  creditos.value = await obtenerNombresCreditos();
  creditos.value = creditos.value.data.body;
  asesores.value = await obtenerTodos();
  asesores.value = creditos.value.data.body;
  await consultarUsuarioAct();
  await montarCotizaciones();
...
const montarCotizaciones = async () => {
cotizacionesFiltrados.value=[];

  cotizaciones.value = await obtenerCotizaciones();
  cotizaciones.value = cotizaciones.value.data.body;
  console.log(cotizaciones.value);

  let cliente;
  cotizaciones.value.forEach(async (element) => {
    ...
    element.asesores = await obtenerTodos(element.AsesoresBAZ_idAsesoresBAZ);
    element.asesores = element.medio.data.body[0].Descripcion;
    ...
    if (superUsuario.value || element.usuario.trim() == nickActual.value.trim()) {
        cotizacionesFiltrados.value.push(element)
    }
    prospectosDesplegados.value=cotizacionesFiltrados.value;
  });
...
const actualizarTabla = (nombre) => {
  if (nombre == "") {
    cotizacionesFiltrados.value=prospectosDesplegados.value;
  } else {
  ...
  <div class="container-fluid">
    ...
    <div class="table-responsive-xl">
      ...
      >
        ...
        <tbody>
          <tr v-for="prospecto in cotizacionesFiltrados" :key="prospecto.idProspectos">
